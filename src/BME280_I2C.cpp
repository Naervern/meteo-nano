...
#include "BME280_I2C.h"
    ...
    float BME280_I2C::getPressure(){ //returns pressure in hPa
        readData();
        press_cal = calibration_P(pres_raw);
        press_act = (double)press_cal / 100.0;
    ...
    float BME280_I2C::getHumidity(){ //returns relative atmospheric humidity in %
        readData();
        hum_cal = calibration_H(hum_raw);
        hum_act = (double)hum_cal / 1024.0;
    ...
    void BME280_I2C::readData(){
        ...
        void BME280_I2C::writeReg(uint8_t reg_address, uint8_t data){
            Wire.beginTransmission(ADDRESS);
